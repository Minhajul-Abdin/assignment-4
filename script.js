...
const allapplicationsection = document.getElementById("all-application-section");
...
*/
  ...
  if (event.target.classList.contains('del-btn')){
    ...
    const removeBtn = parentN.querySelector('.applications-container');
    removeBtn.remove();
...
mainConainer.addEventListener('click', function(event){
    ...
    parentN.querySelector('.job-state').innerText = 'INTERVIEW';
    parentN.querySelector('.job-state').classList.remove('bg-[#EEF4FF]', 'bg-red-200');
    ...
    const jobMatch = interviewList.find(item => item.jobTitle == alljobinfo.jobTitle);
    rejectedList = rejectedList.filter(item => item.jobTitle != alljobinfo.jobTitle);
    ...
    const jobMatch = rejectedList.find(item => item.jobTitle == alljobinfo.jobTitle);
    interviewList = interviewList.filter(item => item.jobTitle != alljobinfo.jobTitle);
...
function makecardelement(){
    ...
    div.className = 'applications-container w-full flex justify-between p-6 bg-white rounded mb-4';
    div.innerHTML = `
    <div class="aplication-info">
          ...
          <div class="del-btn">
            <button
              ...
              class="btn btn-soft p-3 bg-white rounded-full"
            >
...
function interviewCount(){
  ...
  const changedCountInterview = document.getElementById('changed-count-interview');
